...
/**
  * @author y15079
  * @create 2018-03-13 17:12
  * @desc 自己管理offset  保存偏移量在zookeeper上，这种方式貌似已经过时了
  **/
class KafkaManager(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  def createDirectStream[K: ClassTag, V: ClassTag, KD<: Decoder[K]: ClassTag, VD <: Decoder[V] : ClassTag](ssc: StreamingContext, kafkaParams: Map[String, String], topics: Set[String]): InputDStream[(K,V)] = {
    ...
    //从zookeeper上读取offset前先根据实际情况更新offsets
    ...
    topics.foreach(topic => {
      ...
      if (hasConsumed) { //消费过
        /**
          ...
          * 针对这种情况，只有判断一下zk上的consumerOffsets和earliestLeaderOffsets已过时，
